...
static unsigned char aes_key[AES_KEY_SIZE] = "bbs.gxgk.cc";
...
{	
	strcpy((char *)aes_key, "bbs.gxgk.cc");
	if (Config.m_Changepwd == 1)
	{
		u_char mac[6];
		char mac2[18];
		zte.GetMacIP(Config.m_csNetCard, NULL, mac);
		sprintf(mac2, "%02x%02x%02x%02x%02x%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
		strcpy((char *)aes_key, mac2);
	}
	aes_context context;
...
void aes_change(const char *plaintext)
{
	Config.m_Changepwd = 0;
